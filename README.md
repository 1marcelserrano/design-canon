...
Sai um bloco `@theme` pronto pro `globals.css`, com seus valores exatos e fallback de render anexado às fontes (porque a house face some em ambiente headless):
...
`primary` continua `primary`. Nenhuma cor renomeada, nenhuma rampa inventada. Rodada completa (a mesma marca sob a disciplina Vignelli e um DS de cliente sob a suíça) em [examples/worked-example.md](./examples/worked-example.md).
